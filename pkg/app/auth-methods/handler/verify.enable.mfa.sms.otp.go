...
func (h Handler) VerifyEnableMfaSmsOtp(w http.ResponseWriter, r *http.Request) {
	...
	err = h.AuthMethodSrv.VerifyEnableMfaSmsOtp(context.Background(), req.OtpID, req.OtpCode, accessTokenClaim.UserID)
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(message.ErrorMessage{Message: err.Error()})
		return
